...
| Durable local effect | `python -m examples.durable_effects` | `same_receipt=True drafts=1` | SQLite transaction rollback, lost reply, process restart, abrupt exit, concurrent retries, tenant keys, payload conflicts and revocation. No remote transaction or authentication service. |
| Serving load simulation | `python -m examples.serving_load` | Six synthetic load scenarios; fast-link reuse: 35/60 within SLOs. | FIFO link/compute event timelines, memory and slot admission, drain accounting and rejected requests. No measured GPU rates or batching. |
| Adoption analysis | `python -m examples.adoption_analysis` | Reproduces the Chapter 59 paired comparison and canary-only decision. | Independent paired uncertainty, Wilson bound, rare-event bound, timeouts, invalid data and failed gates. Synthetic tasks, not model measurements. |
| Persistent task recovery | `python -m examples.task_recovery` | `status=done attempts=1 drafts=1` | Separate task/effect databases, exhausted-budget reconciliation, process restart, stale generation, revocation, immutable intent and schema checks. Single active runner. |
